...
class EasyYopmail {
  ...
  async readMessage(mail, id, format) {
    let selector = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '';
    try {
      const {
        cookie
      } = await utils.getCookiesAndYP();
      ...
      let message;
      if (selector) {
        selector = "".concat(constants.SELECTOR_MAIL, " ").concat(selector);
  ...
  async writeMessage(mail, to, subject, body) {
    try {
      ...
      if (response.statusCode === 200) {
        const data = {
          msgfrom: "".concat(mail, "@yopmail.com"),
          msgto: to,
          msgsubject: subject,
          msgbody: body
        };
