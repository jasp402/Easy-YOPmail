...
function request(method, url) {
  ...
  const settings = _objectSpread({
    ...
    // Extrae solo el nombre de dominio
    port: 443,
    // Puerto por defecto para HTTPS
    path: "".concat(parsedUrl.pathname).concat(parsedUrl.search),
    // Usa la ruta extraída de la URL
...
async function getYJ(cookie, ver) {
  const response = await request('GET', constants.WEBMAIL_JS_URL(ver), {
    headers: {
      Cookie: cookie
    }
  });
...
function shouldIncludeEmail(email, filteredSearch) {
  return Object.entries(filteredSearch).every(_ref => {
    let [key, value] = _ref;
    switch (key) {
      case 'id':
...
async function detailInbox(mail, yp, yj, ver, cookie) {
  let search = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : {};
  let settings = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : {};
  const pageNumber = 1;
  ...
  while (hasNextPage && (settings.GET_ALL_MAILS === true || currentPage === 1)) {
    ...
    mailFromPage["page_".concat(currentPage)] = currentPageEmails.length;
